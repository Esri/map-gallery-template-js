...
function buildMapPlaylist(obj,data){
	...
	if(totalItems > 0){
		...
		for(var i=0; i < forTotal; ++i) {
			...
			if(data.results[i].url){
				...
				// IF IOS DEVICE
				if(obj.mobileAppLink && (pmgConfig.agent_ios || pmgConfig.agent_android || pmgConfig.agent_winphone)){
					// SHOW IOS LINK
			...
			if(obj.layout == 'list'){
				...
				// BUILD LIST ITEM
				html += '<li class="item"><a class="block" id="mapItem'+i+'" title="' + data.results[i].snippet + '" href="' + itemURL + '"><img src="' + obj.arcgispath + obj.imagepath + data.results[i].id + obj.imagepath2 + data.results[i].thumbnail + '" width="200" height="133"></a><a class="title" id="mapItemLink'+i+'" title="' + data.results[i].snippet + '" href="' + itemURL + '"><strong>' + itemTitle + '</strong></a><p>' + data.results[i].snippet + '</p><p><a class="viewMap" title="View map" href="' + itemURL + '">View map<span class="arrow"></span></a></p><div class="clear"></div></li>';
			}
			else{
				var endRow = false;
				var itemClass = '';
				...
				html += '<a id="mapItem'+i+'" title="' + data.results[i].snippet + '" href="' + itemURL + '"><img src="' + obj.arcgispath + obj.imagepath + data.results[i].id + obj.imagepath2 + data.results[i].thumbnail + '" width="200" height="133"><span>' + itemTitle + '</span></a>';
				html += '</li>';
				if(endRow){
	...
	if(obj.mobileAppLink && pmgConfig.agent_ios){
		for(var i=0; i < forTotal; ++i) {
			document.getElementById("mapItem" + i).onclick = applink(pmgConfig.iosAppURL);
			if(obj.layout == 'list'){
				document.getElementById("mapItemLink" + i).onclick = applink(pmgConfig.iosAppURL);
			}
		}
	}
	// CREATE PAGINATION
