...
function queryArcGISGroupInfo(obj){
	...
	if(settings.id_group){
		...
		$.ajax({
			// AJAX URL
			url: 'http://www.arcgis.com/sharing/community/groups/' + settings.id_group,
...
function queryArcGISGroup(obj){
	...
	var settings = {
		// ARCGIS.COM URL
		arcgispath : 'http://www.arcgis.com/',
		// SEARCH PATH
...
function createPagination(obj,totalItems,pagObject){
	...
	if(obj.pagination && obj.perPage && totalItems > obj.perPage){
		...
		if(obj.paginationShowPrevNext){
			...
			pagination += '<li class="previous '+firstClass+'" data-offset="'+ previous +'"><span class="silverButton buttonLeft"><span></span></span></li>';
		}
		if(obj.paginationShowFirstLast && current > (obj.paginationSize + 1)){
			pagination += '<li data-offset="'+first+'"><span class="default">'+first+'</span></li><li><sub>&hellip;</sub></span></li>';
		}
		// CREATE EACH PAGINATION ITEM
		for(var i=1; i <= Math.ceil(totalItems / obj.perPage); ++i) {
			if(i <= (current + obj.paginationSize) && i >= (current - obj.paginationSize)){
				...
				// PAGE LIST ITEM
				pagination += '<li data-offset="'+i+'" class="'+selectedClass+'"><span class="default">'+i+'</span></li>';
			}
		}
		// PAGINATION AFTER
		if(obj.paginationShowFirstLast && current < (last - obj.paginationSize)){
			pagination += '<li><sub>&hellip;</sub></span></li><li data-offset="'+last+'"><span class="default">'+last+'</span></li>';
		}
		// PAGINATION NEXT
		if(obj.paginationShowPrevNext){
			...
			pagination += '<li class="next '+lastClass+'" data-offset="'+next+'"><span class="silverButton buttonRight"><span></span></span></li>';
		}
		// END PAGINATION
...
function getViewerURL(){
	switch(pmgConfig.mapViewer){
		case 'arcgis':
			return 'http://www.arcgis.com/home/webmap/viewer.html?webmap=';
			break;
		case 'explorer':
